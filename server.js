...
function viewEmployeesByDepartment() {
  connection.query(qryEmp, function (err, res) {
    if (err) throw err;
    console.table("Enter the ID of a department to view the employees:", res);
});
  ...
  inquirer
    .prompt({
      name: "id",
      type: "input",
      message: "Enter the ID of a department to view the employees:",
    })
    .then((answer) => {
      let qryDep2 = qryDep + " WHERE a.department_id = " + parseInt(answer.id);
      connection.query(qryDep2, function (err, res) {
          if (err) throw err;
          console.table("Employees from the selected department:", res);
          mainMenu();
      });
  });
};
